...
from utils import get_imagenet_trained_teacher_for_cub_teacher, LabelSmoothing
...
# global variable to store best top1 accuracy
best_acc1 = 0
best_acc5 = 0
...
def parse():
    ...
    parser.add_argument('--wd', '--weight-decay', default=1e-4, type=float,
                        ...
                        dest='weight_decay')
    parser.add_argument('-p', '--print-freq', default=24, type=int,
    ...
    parser.add_argument('--output_dir', default='/results_cub/', type=str,
    ...
    # LS
    parser.add_argument('--alpha', default=0.0, type=float, help='label smoothing value')

    # For finetuning
    parser.add_argument('--imagenet_weights_path', type=str, required=True,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # create model
    if args.pretrained:
        model, teacher_model_name = get_imagenet_trained_teacher_for_cub_teacher(args)
    else:
    ...
    if args.alpha > 0:
        print("Using \u03B1 = ", args.alpha)
        criterion_train = LabelSmoothing(smoothing=args.alpha).cuda(args.gpu)
    else:
    ...
    # Use ImageNet settings (We use Transfer Learning)
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                     std=[0.229, 0.224, 0.225])
    

    # Data loaders
    # Add more augmentations (Larger variance)
    ...
    print("CUB 200-2011 dataset statistics: #train images = {}, #val images = {}".format(
    ...
    if not args.multiprocessing_distributed or (args.multiprocessing_distributed
        ...
        tags=[ 'teacher', str(args.alpha) ], group='cub200-2011')
        wandb.config.update(args)
        wandb.watch(model, log="gradients", log_freq=24)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        tr_loss, tr_top1, tr_top5, current_lr = train(train_loader, model, criterion_train, optimizer, scaler, epoch, args)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            "train/top1_acc": tr_top1, 
            "train/top5_acc": tr_top5,
            ...
            }, is_best, filename=os.path.join(args.output_dir, 'cub-teacher={}-{}-checkpoint.pth.tar'.format(args.arch, args.alpha)))
